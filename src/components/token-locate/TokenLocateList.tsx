...
export default function TokenLocateList({ result }: { result: LocateResponse }) {
  ...
  const rawJupiterSections = result.rawSections.filter((s) => s.dataTier === 'raw')
  const enrichedJupiterSections = result.rawSections.filter(
    (s) => s.dataTier === 'jupiter_enriched',
  )
  const internalSections = result.rawSections.filter((s) => s.dataTier === 'internal')
