...
export default function JupiterTerminal({ 
  ...
  const handleSwapSuccess = useCallback(async ({ txid, swapResult, quoteResponseMeta }: SwapSuccessData) => {
    ...
    if (!walletContextState.publicKey || !trackOperation) {
      console.warn('Cannot track swap: wallet not connected or trackOperation not available')
      return
    }
    ...
    try {
      ...
      const inputUIAmount = swapResult.inputAmount / Math.pow(10, swapResult.inputDecimals || 9)
      const outputUIAmount = swapResult.outputAmount / Math.pow(10, swapResult.outputDecimals || 9)
      ...
      if (swapResult.inputAddress === SOL_MINT) {
        ...
        tokenInfo = {
          ...
          name: swapResult.outputName || swapResult.outputSymbol || 'Unknown Token',
          logoURI: swapResult.outputLogoURI,
          tokenAmount,
          solAmount,
          priceUsd: quoteResponseMeta?.quoteResponse?.outputTokenPrice || (tokenAmount > 0 ? (solAmount * solPriceUsd) / tokenAmount : 0),
        ...
        tokenInfo = {
          ...
          name: swapResult.inputName || swapResult.inputSymbol || 'Unknown Token',
          logoURI: swapResult.inputLogoURI,
          tokenAmount,
          solAmount,
          priceUsd: quoteResponseMeta?.quoteResponse?.inputTokenPrice || (tokenAmount > 0 ? (solAmount * solPriceUsd) / tokenAmount : 0),
          solPrice: solPriceUsd
        }
      } else {
        // Token -> Token (treat as sell of input token)
        operationType = 'sell'
        solAmount = inputUIAmount * (quoteResponseMeta?.quoteResponse?.inputTokenPrice || 0) / solPriceUsd
      ...
      if (tokenAmount <= 0) {
        ...
        tokenAmount = operationType === 'buy' ? outputUIAmount : inputUIAmount
      }
