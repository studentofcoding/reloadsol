...
export default function ChartBuyModal({ tokenAddress, onClose, initialBuyAmount = '0.01' }: ChartBuyModalProps) {
  ...
  const gmgnChartUrl = tokenAddress ? `https://www.gmgn.cc/kline/sol/${tokenAddress}?interval=1H` : ''
  ...
  }, [connected, publicKey, signAllTransactions, connection, buyAmount, tokenAddress, slippage, priorityFee, tokenInfo, trackOperation, loadUserTokens, initialBuyAmount])
  ...
  const OHLCChart = ({ bars, height = 400 }: { bars: OHLCBar[]; height?: number }) => {
    const data = {
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                ...
                {riskInfo && (
                  <span className={`px-2 py-0.5 rounded text-xs border ${getRiskBadgeColor(riskInfo.overallRisk)}`}>
                    {riskInfo.overallRisk} RISK
                  </span>
                )}
              </h2>
              <div className="text-sm text-gray-400 font-mono">{tokenAddress}</div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-4">
               ...
               <div className="flex-1 relative">
                {chartMode === 'ohlc' ? (
                    <>
                        ...
                        {!isOhlcLoading && !ohlcError && <OHLCChart bars={ohlcBars} height={500} />}
                        {ohlcError && <div className="flex items-center justify-center h-full text-red-400">{ohlcError}</div>}
                    </>
                ) : (
                    <iframe 
                        src={gmgnChartUrl}
            ...
            {tokenInfo?.marketCap && (
                <div className="bg-gray-800 rounded-lg p-4">
                    <RiskAnalysis tokenAddress={tokenAddress} marketCap={tokenInfo.marketCap} defaultExpanded={false} />
                </div>
            )}
          </div>
          ...
          <div className="space-y-4">
             ...
             <div className="bg-gray-800 rounded-lg p-4 grid grid-cols-2 gap-2">
                <div>
                    ...
                    <div className="text-white font-mono">${tokenInfo?.price?.toFixed(8) || '...'}</div>
                </div>
                <div>
                    <div className="text-xs text-gray-400">MCap</div>
                    <div className="text-white font-mono">${tokenInfo?.marketCap?.toLocaleString() || '...'}</div>
                </div>
             </div>
                     ...
                     {isLoadingPositions ? (
                         ...
                         <div className="space-y-1">
                             <div className="flex justify-between text-sm">
                                 ...
                                 <span className="text-white">{currentPosition.uiAmount.toLocaleString()} {tokenInfo?.symbol}</span>
                             </div>
                             <div className="flex justify-between text-sm">
                                 <span className="text-gray-400">Value:</span>
                                 <span className="text-white">${currentPosition.usdValue?.toFixed(2) || '0.00'}</span>
                             </div>
                         </div>
                     ) : (
                    ...
                    <>
                        ...
                        {showAdvanced && (
                            <div className="grid grid-cols-2 gap-2 bg-gray-700/30 p-2 rounded">
                                ...
                                <div>
                                    <label className="text-xs text-gray-500">Priority Fee</label>
                                    <select
                                        value={priorityFee}
