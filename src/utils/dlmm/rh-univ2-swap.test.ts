...
import {
  buildRhUniv2BuyCalls,
  buildRhUniv2SellCalls,
...
describe('buildRhUniv2BuyCalls', () => {
  it('encodes ETH buy with value', () => {
    ...
    expect(calls).toHaveLength(1)
    expect(calls[0]!.to).toBe(RH_V2_ROUTER)
    expect(calls[0]!.value).toBe(AMOUNT_IN)
    expect(calls[0]!.data).toBe(
      encodeFunctionData({
        abi: univ2RouterAbi,
        functionName: 'swapExactETHForTokens',
        args: [MIN_OUT, [RH_WETH, TOKEN], ACCOUNT, DEADLINE],
      }),
    )
  })

  it('encodes USDG buy without value', () => {
    const calls = buildRhUniv2BuyCalls({
      token: TOKEN,
      account: ACCOUNT,
      amountIn: AMOUNT_IN,
    ...
    expect(calls[0]!.data).toBe(
      encodeFunctionData({
        ...
        args: [AMOUNT_IN, MIN_OUT, [RH_USDG, TOKEN], ACCOUNT, DEADLINE],
      }),
    )
  })
})
