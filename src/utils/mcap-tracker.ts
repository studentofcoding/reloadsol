...
export interface McapTrackingResult {
    ...
    firstSeenAt?: string // Add timestamp for when token was first tracked
}
...
export async function trackTokenMcap(
    ...
    try {
        ...
        if (cached && (now - new Date(cached.last_updated_at).getTime()) < CACHE_TTL_MS) {
            ...
            if (Math.abs(cached.current_mcap - currentMcap) > cached.current_mcap * 0.01) { // 1% threshold
                cached.current_mcap = currentMcap
                cached.last_updated_at = new Date().toISOString()
                cached.mcap_growth_percent = ((currentMcap - cached.first_mcap) / cached.first_mcap) * 100
        ...
        if (error && error.code !== 'PGRST116') { // PGRST116 = no rows returned
            ...
            return { isFirstTime: true, currentMcap }
        }
        ...
        if (existingRecord) {
            ...
            const growthPercent = ((currentMcap - existingRecord.first_mcap) / existingRecord.first_mcap) * 100
            ...
            if (Math.abs(existingRecord.current_mcap - currentMcap) > existingRecord.current_mcap * 0.01) {
