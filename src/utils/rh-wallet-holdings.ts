...
// ---------------------------------------------------------------------------
// Shared per-token USD pricing + metadata (used by both /api/rh/wallet-tokens
// tiers and the Goldsky ledger path). Single GMGN call per token, Redis-cached.
// ---------------------------------------------------------------------------
...
export async function fillMissingRhUsd(
  ...
  for (let i = 0; i < missing.length; i += concurrency) {
    ...
    try {
      ...
      if (error instanceof GmgnApiError && error.code === 'RATE_LIMIT') {
        console.warn('[rh-wallet-holdings] price fill rate limited, partial')
        break
      }
      throw error
    }
