...
interface FilterOptions {
  ...
  minDuration: string
  maxDuration: string
  sortBy: 'created_at' | 'peak_gain_percentage' | 'current_gain_percentage' | 'tracking_duration' | 'status_changed_at'
  sortOrder: 'asc' | 'desc'
}
...
export default function TrackingHistoryPage() {
  ...
  const [selectedToken, setSelectedToken] = useState<TrackedTokenHistory | null>(null)
  ...
  const getStatusColor = (status: string): string => {
    switch (status) {
      case 'won': return 'bg-green-600 text-green-100'
      case 'lost': return 'bg-red-600 text-red-100'
      case 'tracking': return 'bg-blue-600 text-blue-100'
      case 'waiting': return 'bg-yellow-600 text-yellow-100'
      case 'skipped': return 'bg-gray-600 text-gray-100'
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {stats && (
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-6">
              ...
              <div className="bg-blue-900/20 border border-blue-600 p-4 rounded-lg text-center">
                <div className="text-2xl font-bold text-blue-400">{stats.tracking}</div>
          ...
          <div className="bg-gray-800 rounded-lg p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="current_gain_percentage">Current Gain %</option>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Min Gain %</label>
                <input
                  type="number"
                  value={filters.minGain}
                  onChange={(e) => setFilters(prev => ({ ...prev, minGain: e.target.value }))}
                  placeholder="e.g. -50"
                  className="w-full p-3 bg-gray-700 border border-gray-600 text-white rounded-md focus:ring-2 focus:ring-blue-500"
                />
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Max Gain %</label>
                <input
                  type="number"
                  value={filters.maxGain}
                  onChange={(e) => setFilters(prev => ({ ...prev, maxGain: e.target.value }))}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Min Duration (hours)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Max Duration (hours)</label>
                <input
                  type="number"
          ...
          <div className="space-y-4">
            {tokens.map((token) => (
              <div key={token.id} className="bg-gray-800 rounded-lg p-6 hover:bg-gray-750 transition-colors">
                {/* Token Header */}
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center space-x-3">
                    {token.logo_url ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  ...
                  <div className="bg-gray-700 p-3 rounded-lg">
                    ...
                    <div className="text-lg font-bold text-white">
                      ${formatPrice(token.initial_price_usd)} → ${formatPrice(token.last_price_usd)}
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-700 p-3 rounded-lg">
                    ...
                    <div className={`text-lg font-bold ${
                      token.current_gain_percentage >= 0 ? 'text-green-400' : 'text-red-400'
                    }`}>
                  ...
                  <div className="bg-gray-700 p-3 rounded-lg">
                    ...
                    <div className={`text-lg font-bold ${
                      token.peak_gain_percentage >= 0 ? 'text-green-400' : 'text-red-400'
                    }`}>
                ...
                <div className="mt-4 pt-4 border-t border-gray-700">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <span className="text-white ml-2">
                        {new Date(token.tracking_started_at).toLocaleDateString()}
                      </span>
                    </div>
                    <div>
                      ...
                      <span className="text-white ml-2">
                        {token.status_changed_at 
                          ? new Date(token.status_changed_at).toLocaleDateString()
                          : 'Still active'
                        }
                      </span>
                    </div>
              ...
              <div className="bg-gray-800 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                <div className="p-6">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="bg-gray-700 p-4 rounded-lg">
                        ...
                        <div className="space-y-2 text-sm">
                          <div><span className="text-gray-400">Symbol:</span> <span className="text-white">{selectedToken.token_symbol}</span></div>
                          <div><span className="text-gray-400">Name:</span> <span className="text-white">{selectedToken.token_name}</span></div>
                          <div><span className="text-gray-400">Address:</span> <span className="text-white font-mono text-xs">{selectedToken.token_address}</span></div>
                          <div><span className="text-gray-400">Status:</span> <span className={`font-medium ${getStatusColor(selectedToken.status)}`}>{selectedToken.status.toUpperCase()}</span></div>
                        </div>
                      </div>
                      ...
                      <div className="bg-gray-700 p-4 rounded-lg">
                        ...
                        <div className="space-y-2 text-sm">
                          <div><span className="text-gray-400">Initial Price:</span> <span className="text-white">${formatPrice(selectedToken.initial_price_usd)}</span></div>
                          <div><span className="text-gray-400">Last Price:</span> <span className="text-white">${formatPrice(selectedToken.last_price_usd)}</span></div>
                          <div><span className="text-gray-400">Peak Price:</span> <span className="text-white">${formatPrice(selectedToken.peak_price_usd)}</span></div>
                          <div><span className="text-gray-400">Current Gain:</span> <span className={selectedToken.current_gain_percentage >= 0 ? 'text-green-400' : 'text-red-400'}>{formatPercentage(selectedToken.current_gain_percentage)}</span></div>
                          <div><span className="text-gray-400">Peak Gain:</span> <span className={selectedToken.peak_gain_percentage >= 0 ? 'text-green-400' : 'text-red-400'}>{formatPercentage(selectedToken.peak_gain_percentage)}</span></div>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Market Data */}
                    <div className="bg-gray-700 p-4 rounded-lg">
                      <h3 className="text-lg font-bold text-white mb-3">Market Data</h3>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div><span className="text-gray-400">Market Cap:</span> <span className="text-white">{selectedToken.market_cap ? `$${formatNumber(selectedToken.market_cap)}` : 'N/A'}</span></div>
                        <div><span className="text-gray-400">Organic Score:</span> <span className="text-white">{selectedToken.organic_score?.toFixed(2) || 'N/A'}</span></div>
                        <div><span className="text-gray-400">Volume 1h:</span> <span className="text-white">{selectedToken.volume_1h ? `$${formatNumber(selectedToken.volume_1h)}` : 'N/A'}</span></div>
                      </div>
                    </div>
                    
                    {/* Timing Information */}
                    <div className="bg-gray-700 p-4 rounded-lg">
                      <h3 className="text-lg font-bold text-white mb-3">Timing Information</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div><span className="text-gray-400">Created:</span> <span className="text-white">{new Date(selectedToken.created_at).toLocaleString()}</span></div>
                        <div><span className="text-gray-400">Tracking Started:</span> <span className="text-white">{new Date(selectedToken.tracking_started_at).toLocaleString()}</span></div>
                        <div><span className="text-gray-400">Status Changed:</span> <span className="text-white">{selectedToken.status_changed_at ? new Date(selectedToken.status_changed_at).toLocaleString() : 'Still active'}</span></div>
                        <div><span className="text-gray-400">Duration:</span> <span className="text-white">{formatDuration(selectedToken.tracking_started_at, selectedToken.status_changed_at)}</span></div>
                      </div>
                    </div>
                    ...
                    {selectedToken.trade_comparison_data && (
                      <div className="bg-gray-700 p-4 rounded-lg">
                        <h3 className="text-lg font-bold text-white mb-3">Trade Comparison</h3>
                        <div className="text-sm text-gray-300">
                          <pre className="whitespace-pre-wrap overflow-x-auto">
                            {JSON.stringify(selectedToken.trade_comparison_data, null, 2)}
                          </pre>
                        </div>
                      </div>
                    )}
                    ...
                    {selectedToken.trading_simulation && (
                      <div className="bg-gray-700 p-4 rounded-lg">
                        <h3 className="text-lg font-bold text-white mb-3">Trading Simulation</h3>
                        <div className="text-sm text-gray-300">
                          <pre className="whitespace-pre-wrap overflow-x-auto">
                            {JSON.stringify(selectedToken.trading_simulation, null, 2)}
                          </pre>
                        </div>
                      </div>
                    )}
                    ...
                    {/* Price History */}
                    {selectedToken.price_history && selectedToken.price_history.length > 0 && (
                      <div className="bg-gray-700 p-4 rounded-lg">
                        <h3 className="text-lg font-bold text-white mb-3">Price History</h3>
                        <div className="space-y-2 max-h-40 overflow-y-auto">
                          {selectedToken.price_history.map((record: any, index: number) => (
                            <div key={index} className="flex justify-between text-sm">
                              <span className="text-gray-400">{new Date(record.timestamp).toLocaleString()}</span>
                              <span className="text-white">${formatPrice(record.price_usd)}</span>
                            </div>
                          ))}
                        </div>
                      </div>
