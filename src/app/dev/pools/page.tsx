...
export default function TrendingPoolsPage() {
  ...
  const handleChartClick = useCallback((token: TrendingToken, event: React.MouseEvent) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!target.closest('.chart-tooltip') && !target.closest('.chart-button')) {
        setSelectedToken(null)
        setTooltipPosition(null)
      }
    }
          ...
          {trendingData && (
            <div className="space-y-6">
              {MARKET_CAP_CATEGORIES.map(category => {
                ...
                return (
                  <div key={category.name} className={`border rounded-lg p-6 ${category.bgColor}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {categoryTokens.map((token, index) => {
                        ...
                        return (
                          <div key={`${token.token_address}-${index}`} className="bg-gray-800 border border-gray-600 rounded-lg p-4 hover:border-gray-500 transition-colors relative">
                            {/* Token Header */}
                            ...
                            <div className="space-y-2 text-sm">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className="text-green-400 font-bold">${formatNumber(token.mcap)}</span>
                              </div>
        ...
        >
          <div className="p-4">
            ...
            <div className="flex gap-2">
              <button 
                onClick={() => window.open(`https://www.gmgn.cc/kline/sol/${selectedToken.token_address}?interval=1D`, '_blank')}
