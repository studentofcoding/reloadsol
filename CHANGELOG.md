...
- **Token stats search used a retired host.** `searchTokenStats` called `https://tokens.jup.ag/token/<mint>`, which no longer resolves (`getaddrinfo ENOTFOUND`, logged as `Error searching token stats` on every request). It now reuses [`fetchTokenMetadataFromJupiter`](src/utils/jupiter-metadata.ts) — `lite-api.jup.ag` Token API v2, throttled, retries 429/504.
- **`/api/logs` read a store nothing wrote to.** `api-logger.ts` was imported only by `/api/logs` and `/api/logs/stream`, while every strategy file logs through `unified-logger` — whose buffer had no reader and whose text output goes through `console.log` (stripped from production builds by `removeConsole`). Both routes now read `unified-logger` and the dead `api-logger.ts` is deleted (335 lines, zero importers). Every existing `log.*` call across ~19 strategy files becomes observable for the first time.
- **24-hour auto-overwrite on the log buffer:** `LOG_BUFFER_TTL_HOURS` (default **24**, env-tunable) drops expired entries on every write, `MAX_LOG_ENTRIES` remains the hard ceiling, and `getLogStats()` reports `retentionHours`.
- **Trending-cycle diagnostics are no longer invisible in production:** the GMGN candidate count, cycle start, window allowed/restricted and the 5-minute summary now go to the unified buffer (`GET /api/logs?level=info`) instead of `console.log`.

### Fixed — the trending trading window now matches what it always claimed

- `isWithinTradingHours()` admitted **15:00–05:59 GMT+7** (`hour >= 15 || hour < 6`) while its own comment, the Discord post, the 403 body and [`docs/architecture.md`](docs/architecture.md) all said **16:00–04:00** — an extra hour at each end. The window is now defined once (`TRADING_START_HOUR` / `TRADING_END_HOUR`) with `TRADING_HOURS_LABEL` exported and used by every message, so the enforced and displayed hours cannot drift apart again.
- Evidence (`strategy_outcomes`, entry hour in GMT+7, measured 2026-09-25): of ~82k outcomes only `scalper` (185) and `att` (50) are actually gated — every other strategy has entries in hours 06–14, which the window forbids. In that gated cohort the extra hours were materially worse: `scalper` **−15.1%** avg PnL across 15:00/04:00/05:00 vs **−2.8%** in-window, `att` −39.1% vs −11.9% (n=57 edge entries — directional, not conclusive).
- Related findings, deliberately **not** changed: `runTrendingBotRhSimCycle()` runs **before** the gate ([`cycle.ts:120`](src/strategies/trending-track/cycle.ts) vs `:156`), so `att_rh` — 94% of all outcomes — has **22,948 of its 76,941 entries (29.8%) in the hours the window claims to close**, though its PnL is flat across those bands (−0.08 vs −0.07). And the best avg-PnL hours in the whole table (hr 08–10 and 14–15, +4.4% to +11.7%) sit mostly *inside* the forbidden band, driven by the profitable ungated strategies (`mcap_enter_first_seen` +44%, `signals_sell_over_100` +84%). Extending the gate to them is a design question this data does not settle.
