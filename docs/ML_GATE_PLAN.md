...
4. Potential model: advisory by default (`ML_POTENTIAL_EXIT_MODE=shadow`); sim TP/SL overlay via [`potential-exit-overlay.ts`](../src/strategies/potential-exit-overlay.ts) when `apply` only if `metrics.potential_ready === true` — see [reloadsol_engine_strategies_and_ml.md](./reloadsol_engine_strategies_and_ml.md)
