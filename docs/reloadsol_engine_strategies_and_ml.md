...
**Monitor snapshots / volume:** `resolveTokenMonitorSnapshot` fills `price_usd` + `volume_5m` via tracker → `token_mcap_tracking.volume_5m` → Jupiter v2 (`usdPrice`, stats windows buy+sell, `mcap`). Entry `volume_at_entry` waterfall: local/monitor → Jupiter `stats5m→1h→6h→24h` (stamps `volume_at_entry_window`) → DexScreener best-pair `m5→h1→h24` (`volume_at_entry_source: dexscreener`). **V1 volume is optional:** missing/non-finite → `log_volume_at_entry=0` (does not block extract); telemetry `volume_imputed`. Signals/trending closes call `ensureCompleteBuyFeaturesForOutcome`. `monitor_snapshots` are path/series data (not V1 gate inputs); do not rewrite historical null ticks. Ops: `POST /api/strategies/ml/backfill-features?limit=15` (nginx-safe); response includes `volume_filled_from` + post-merge `sample_missing_fields`; dataset-stats exposes `incomplete_by_field` + `volume_imputed`. Retrain gate/potential after impute lands.
