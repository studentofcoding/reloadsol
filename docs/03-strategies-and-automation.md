...
| `dlmm_screen` (Hunter) | Meteora DLMM pools | `scorePool` × `solDlmmConfidence`; Redis last-good on fetch fail; `dlmm_candidates` `chain=sol` | 300s | `src/app/api/dlmm/screen/route.ts`, `src/utils/dlmm/screener.ts`, `sol-dlmm-confidence.ts` |
...
- Key env defaults: `TRENDING_TRACKER_SECRET`/`TRIGGER_SECRET` (auth), `TRENDING_FEED` (`gmgn` \| `jupiter`), `TRENDING_REENTRY_COOLDOWN_MIN=1440`, `TRENDING_MAX_PURCHASES_PER_TOKEN=2`, `TRENDING_DROP_RUGGED` (on), `OHLC_SAMPLE_INTERVAL=15`, `OHLC_SAMPLE_MAX_MINTS=300`, `OHLC_BARS_RETENTION_HOURS=48`, `SIGNALS_SIM_INTERVAL=120`, `MCAP_TRACKER_SIM_OPEN_INTERVAL=15`, `MCAP_TRACKER_SIM_INTERVAL=120`, `GMGN_SIM_INTERVAL=120`, `GMGN_ACTIVITY_POLL_INTERVAL=180`, `SOCIAL_ROLLUP_INTERVAL=300`, `DLMM_SCREEN_INTERVAL=300`, `DLMM_MANAGE_INTERVAL=60`, `RH_CLMM_MANAGE_INTERVAL=300`, `SOL_ARB_SCAN_INTERVAL=60`, `STRATEGY_REPORT_INTERVAL=86400`, `STRATEGY_TRACK_TELEGRAM_ENABLED` (global telegram kill switch).
