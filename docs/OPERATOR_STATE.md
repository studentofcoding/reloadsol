...
- Telegram: `sendSignalsEarlyEnterAlert` **only if** matching Signals strategy `config.notify.telegram` is on (default on; Activate syncs on, Deactivate syncs off; manual override allowed). UI toasts follow `config.notify.ui`. Stamps/`signals_early` still run.
...
- **Telegram** only when the strategy currently qualifies as a **best emitter** (rank lock `avg×n+win%`; Researchy min-n: all-time ≥30 or 7d ≥10; below floor = hypothesis only; sum% footnote only) **and** `config.notify.telegram` is on. Message is a **follow alert** (not auto-enter): arm `first_seen`/`80%`, avg×n rank context, chart link (OHLC photo or GMGN URL if bars thin). Mint+arm 24h cooldown. Soft-gate / Noul / paper stay out of this path.
