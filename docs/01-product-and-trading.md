...
|---|---|---|---|
| **Reload (home)** | `/` logged-out; `/sell/solana` or `/sell/robinhood` after connect | Logged-out: branding + wallet chooser. After connect (or brand click / visiting `/` while connected): full sell by `AppNetwork`. | `HomePageClient.tsx`, `connectedSellPath` in `route-network.ts`, `BulkTokenSeller` |
| Bulk buy | `/buy` (`/buy/solana`, `/buy/robinhood`) | Buy up to the chain-specific cap (currently **5 RH / 5 Solana**) from one spend amount (SOL on Solana; ETH/USDG/WETH on RH); valid/parsed chips; risk analysis; trending/toast tokens append to the list. Dev wallets get a thin link to `/dev/insight` for scout. | `src/components/BulkTokenBuyer.tsx`, `src/components/RiskAnalysis.tsx` |
| Insight (dev) | `/dev/insight` | Per-network data-public scout (`buybulk-datapublic-scout`) + climate **Safe** / **Not safe** / **Unknown** with terminal **headline** (e.g. `Chop mode`; tooltip `detail`). Sol also embeds **Roster digger (Sol)**. Paper note is Safe-gated in `strategy_paper_notches` — never live-executes | `src/components/InsightPageClient.tsx`, `src/components/DataPublicObserveStrip.tsx`, `src/components/signals/RosterTab.tsx` |
| Bulk sell / dust sweep / reload | `/sell` (`/sell/solana`, `/sell/robinhood`) | Full seller UI. Post-connect landing from home. Dust categories (sellable / unsellable / zero-balance / frozen / NFT); empty-ATA close + rent reclaim. Full `/sell` can swap the batch into one custom mint (default native SOL/ETH). | `src/components/BulkTokenSeller.tsx`, `src/utils/jupiter.ts`, `src/utils/swap-executor.ts` |
