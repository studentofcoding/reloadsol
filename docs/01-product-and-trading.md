...
| Bulk buy | `/buy` (`/buy/solana`, `/buy/robinhood`) | Buy up to the chain-specific cap (currently **5 RH / 5 Solana**) from one spend amount (SOL on Solana; ETH/USDG/WETH on RH); valid/parsed chips; risk analysis; trending/toast tokens append to the list. **Observe strip** lists filtered data-public RH+Sol candidates; **Paper note** is local interest only and only when climate display is Safe — never live-executes from that feed | `src/components/BulkTokenBuyer.tsx`, `src/components/DataPublicObserveStrip.tsx`, `src/components/RiskAnalysis.tsx` |
