...
| Climate chip on insight | Binary label **Safe** / **Not safe** / **Unknown** (no `Climate ` prefix) plus terminal **headline** (e.g. `Chop mode`). Tooltip prefers **detail**; falls back to `state · H` on old terminals. Header chip stays display-only. |
...
3. Confirm climate chip shows **Not safe** / **Safe** / **Unknown** (no `Climate ` prefix) plus terminal **headline** (e.g. `Chop mode`). Tooltip prefers **detail**; H stays in the tooltip. Scout **score**, **liq**, **mcap**, chain count, and paper-note scores use `@sfinterface/numbers` (they roll when the poll updates).
