...
5. **Runtime inference.** Web container loads artifacts lazily and caches ONNX sessions per process:
   ...
   - Feature vector order must equal `meta.feature_columns` (tensor input built from meta). Deploy verification: `scripts/verify-standalone-build.sh` fails if `.next/standalone/node_modules/onnxruntime-node/bin` lacks `onnxruntime_binding.node` plus `libonnxruntime.so*` (Linux) or `libonnxruntime.dylib*` (Mac ship); `next.config.js` uses `output: 'standalone'` with `outputFileTracingIncludes` for `onnxruntime-node/bin` on the sim-track and pattern-reload routes. Gate/potential ONNX are **not** hot-reloaded — restart `reloadsol-web` after a potential/gate retrain (`docs/OPERATOR_STATE.md` server checklist).
