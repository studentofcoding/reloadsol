...
- Do-not list (`docs/OPERATOR_STATE.md` constraints, `docs/ML_GATE_PLAN.md` risks): don't gate live until `*_ready`; don't change frozen entry/exit rules mid-collection; reject live gating when `metrics.gate_ready` is false; review shadow `ml_gate_p_bad` histograms before flipping enforce.
