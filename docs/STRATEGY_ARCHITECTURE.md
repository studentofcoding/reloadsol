...
- **Best-strategy mint share (Telegram):** not a leaderboard. On Stage-2 mcap open, if the strategy qualifies under locked rank **avg pnl% × n + win%** with Researchy floors (all-time n≥30 or 7d n≥10), send copy-trade Telegram + OHLC (`best-strategies-qualify.ts`, `best-strategies-share-notify.ts`). Below floor → hypothesis/low-n only (cannot win top slots). sum% footnote only. Uses `TELEGRAM_BOT_TOKEN` + `TELEGRAM_ALERT_CHAT_ID`.
