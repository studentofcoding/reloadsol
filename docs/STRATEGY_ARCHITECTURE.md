...
- **Best-strategy follow alert (Telegram):** not a leaderboard and not auto-enter. On Stage-2 mcap open, if the arm qualifies under **avg pnl% × n + win%** + Researchy floors (all-time n≥30 or 7d n≥10), send a **follow alert** naming the arm (`first_seen` / `80%`), avg×n rank context, and chart (OHLC or GMGN URL when bars are thin). Mint+arm cooldown. Soft-gate / Noul / paper out of path. Uses `TELEGRAM_BOT_TOKEN` + `TELEGRAM_ALERT_CHAT_ID`.
